        ...
        if (target != null&&curScene!=null)//跟随Target
        {
            if (transform.position != target.transform.position)
            {
               //对x和y坐标进行限制
                ...
                float y = Mathf.Clamp(target.transform.position.y, -curScene.GetComponent<SpriteRenderer>().bounds.size.y / 2 + GameManager.Instance.viewHeight / 2,/*地图下边缘y加上一半摄像机y*/
                      curScene.GetComponent<SpriteRenderer>().sprite.bounds.size.y / 2 - GameManager.Instance.viewHeight / 2/*地图上边缘y加去一半摄像机y */);
                //再进行lerp
                transform.position = Vector3.Lerp(transform.position, new Vector3(x, y,-10), smooth);
            }
        }
